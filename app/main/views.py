...
def update_codee():
    ...
    get_resp = requests.get(
        f"https://api.github.com/repos/{owner}/{repo}/contents/{codee_path}",
        headers=request_header(session["access_token"]),
    )
    ...
    blob_request_body = {"content": codee_content, "encoding": "utf-8"}
...
def create_codee():
    ...
    if commit_list_resp.ok:
        ...
        resp = requests.put(
            f"https://api.github.com/repos/{owner}/{repo}/contents/{save_location}/{codee_name}.cd",
            data=json.dumps(request_body),
            headers=request_header(session["access_token"]),
        )
